...
public class DishController {
    ...
    public Result startOrStop(@PathVariable("status") Integer status, @RequestParam("id") Long id){
        log.info("起售/停售: status-{}, id-{}", status, id);
     ...
     */
    @GetMapping("/list")
    public Result<List<Dish>> listDishes(@RequestParam("categoryId") Long categoryId){
        log.info("根据分类id查询菜品-categoryId: {}", categoryId);
        List<Dish> dishList = dishService.listDishes(categoryId);
